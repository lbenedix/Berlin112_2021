...
for p in range(35):
    results = twitter.get_user_timeline(screen_name='Berliner_Fw', count=200, include_rts=True, page=p)
    ...
    for tweet in results:
        ...
        if 'Berlin112' in hash_tags:  # and not tweet['in_reply_to_screen_name']:
            t = OrderedDict()
            t['bezirk'] = ''
            t['text'] = tweet['text']
...
json.dump(
    tweets,
    open('Berlin112_2021.json', 'w', encoding='utf8'),
    indent=2,
    ensure_ascii=False,
    sort_keys=False
)
